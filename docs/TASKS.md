...
- `[DONE]` **T1.3** — Define initial database migration: `organizations`, `profiles`, and RBAC with RLS. _(2026-09-08 15:47 SAST)_
...
- `[TODO]` **T1.4** — Implement Auth flow (Login, Sign-up, Protected Route Middleware → **`proxy.ts`**, see ARCHITECTURE.md §4). _Blocked by T1.5._
  ...
  - **Blocker (from T1.3).** INSERT is denied to all user roles on `organizations` and `profiles`, so **a new signup currently has no way to obtain an organization**. T1.4 must add a `SECURITY DEFINER` onboarding RPC that creates an organization and its first profile in one transaction.
